...
def sample_frames(flag, num_frames, dataset_name):
  ...
  root = '/data/anti_face_spoofing/CASIA_FASD/dataset'
  dataroot = 'data/MCIO/txt/'
  test_root = '/data/anti_face_spoofing/few_shot_fas'
  # else:
  #   root = 'data/WCS/frame/'
  ...
  if (flag == 0):  # select the fake images
    data = [[
        i.strip()
        for i in open(dataroot + 'fake.txt').readlines()
    ], []]
    ...
    data = [[],
            [
                i.strip() for i in open(dataroot + 'real.txt').readlines()
            ]]
    ...
    data = [[
        ...
        for i in open(dataroot + 'fake.txt').readlines()
            ...
            [
                ...
                i.strip().replace('frame0', 'frame1')
                for i in open(dataroot + 'real.txt').readlines()
            ]]
